...
The four-cycle reverse multiplicity is now controlled exactly in Lean.
`sum_card_goodSwitchRowFinset_le_avoid` uses every legal four-cycle, not an
arbitrary pair: switching is injective because the output determines the
unique row carrying the checked color and hence reconstructs the source
matching.  Thus the sum of all legal four-cycle incidences is at most the
number of edge-avoiding matchings.  This sharpens the earlier factor-two
formal theorem, but the displayed counterexample has only 177,336 such
incidences versus `5*C = 268,020`, so four-cycles alone provably cannot deliver
...
The tempting monotone reduction to the edge-disjoint case is also false.
`nonextendableQueryForbidden_no_fourRegular_superrelation` is a kernel-checked
counterexample generated by four legal completed queries.  On three rows and
three colors their support is `K_3,3`; on the other eight rows and colors it is
already four-regular, and there are no cross edges.  A simple four-regular
superrelation would have to add an edge incident with a deficient first-block
row, but every possible column outside that block is already saturated.

A successful universal proof must therefore count the four checked-edge union
directly, or isolate and handle the deficient overlap components explicitly.
Neither an individual marginal inequality on the original overlapping graph
nor a blanket regular completion can close the argument.

The exact loss target is now formalized rather than hidden behind a union
bound.  `card_completedFourPath_add_aggregateLoss_eq_query` proves the
partition

```text
|path survivors| + |union of four checked-edge events| = |query matchings|.
```

Consequently `89036 + |loss union| < |query matchings|` is precisely the
remaining aggregate inequality for this bridge.  If the query permanent is at
least 276,640, the weaker linear target

```text
3 * |loss union| <= 2 * |query matchings| + 9,529
```

still suffices.  At the minimum query count this permits 187,603 losses, which
is 3,176 more than the floor of the old two-thirds allowance.  Lean carries
both forms through to contradiction from an alleged eight-round strategy.
