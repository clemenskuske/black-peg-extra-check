...
cofactor is an eight-rook fiber; in the false cofactor the first cycle is
deleted and the next nontrivial cycle is marked. The 30 partition rows above
are closed under those two operations. After the first marked expansion every
child is therefore one of the eight-rook states already solved in three
rounds. This gives four rounds in total. The important induction invariant is
the marked cofactor type, not the number of individual matchings.
